...
@interface CareerDivinationViewController ()<UITableViewDelegate,UITableViewDataSource,CareerDivinationSelectDelegate>{
...
@implementation CareerDivinationViewController
...
{
    CareerDivinationCell *cell = [tableView dequeueReusableCellWithIdentifier:@"list"];
    
    if (!cell) {
        cell = [[CareerDivinationCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:@"list"];
    }
...
- (void)loadData
{
    ...
    [self.infoApi lists:type page:pageIndex num:pageSize complete:^(id result, NSError *error) {
        ...
        else
        {
            ...
            NSLog(@"CareerDivinationViewController Result: %@", result);
